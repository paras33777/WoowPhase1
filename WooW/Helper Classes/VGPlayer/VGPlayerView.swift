...
open class VGPlayerView: UIView {
    ...
    open var playControlButton : UIButton = {
        let button = UIButton(type: .custom)
        return button
    }()
    
    open var reverseControlButton : UIButton = {
        let button = UIButton(type: .custom)
        button.alpha = 0
        return button
    }()
    
    open var forwardControlButton : UIButton = {
        let button = UIButton(type: .custom)
        button.alpha = 0
        return button
    }()
    ...
    open var middleControlView : UIView = {
        let view = UIView()
        view.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 0.5)
        return view
    }()
    ...
    var isCloseBtnHidden = false {
        didSet {
            self.closeButton.isHidden = isCloseBtnHidden
        }
    }
...
//    var portraitPlayerVC: PortraitPlayerVC?
//    var vlcPlayerVC: VLCPlayerVC?
    ...
    open func playStateDidChange(_ state: VGPlayerState) {
        ...
        playControlButton.isSelected = state == .playing
    ...
    open func configurationUI() {
        ...
        configurationMiddleControlView()
        setupViewAutoLayout()
    }
...
//        if vlcPlayerVC != nil {
//            if vlcPlayerVC?.orientation == .portrait{
//                parentView = (self.superview)!
//                viewFrame = self.frame
//            }
//            UIDevice.current.setValue(UIInterfaceOrientation.landscapeRight.rawValue, forKey: "orientation")
//            vlcPlayerVC?.orientation = .landscapeRight
//            vlcPlayerVC?.isHidden = false
            UIDevice.current.setValue(UIInterfaceOrientation.landscapeRight.rawValue, forKey: "orientation")
...
//        } else {
//            if portraitPlayerVC?.orientation == .portrait{
//                parentView = (self.superview)!
//                viewFrame = self.frame
//            }
//            UIDevice.current.setValue(UIInterfaceOrientation.landscapeRight.rawValue, forKey: "orientation")
//            portraitPlayerVC?.orientation = .landscapeRight
//            portraitPlayerVC?.isHidden = false
            
//            let statusBarOrientation = UIApplication.shared.statusBarOrientation
//            if portraitPlayerVC?.orientation ?? .portrait == .portrait{
//                parentView = (self.superview)!
//                viewFrame = self.frame
//            }
            
//            portraitPlayerVC?.orientation = .landscapeRight
//            portraitPlayerVC?.orientation1 = .landscapeRight
            UIDevice.current.setValue(UIInterfaceOrientation.landscapeRight.rawValue, forKey: "orientation")
...
extension VGPlayerView {
    ...
    internal func play() {
        ...
        playControlButton.isSelected = true
    }
    
    internal func pause() {
        playButtion.isSelected = false
        playControlButton.isSelected = false
    }
    
    internal func displayControlAnimation() {
        self.playControlButton.isHidden = false
    ...
    internal func hiddenControlAnimation() {
        ...
        delegate?.vgPlayerView(didDisplayControl: self)
        UIView.animate(withDuration: 0.5, animations: {
            self.playControlButton.alpha = 0
            self.bottomView.alpha = 0
            self.topView.alpha = 0
        }) { (completion) in
            self.playControlButton.isHidden = true
...
extension VGPlayerView {
    ...
    @objc internal func onPlayerButton(_ sender: UIButton) {
        if !sender.isSelected {
            vgPlayer?.play()
            playControlButton.contentHorizontalAlignment = .center
            playControlButton.contentVerticalAlignment = .center
        } else {
            vgPlayer?.pause()
            playControlButton.contentHorizontalAlignment = .fill
            playControlButton.contentVerticalAlignment = .fill
        }
    }
    ...
    @objc open func onDoubleTapGesture(_ gesture: UITapGestureRecognizer) {
        print("gesture : \(gesture.numberOfTouches)")
        
        var current: CGFloat = 0
        var totalDuration: CGFloat = 0
        let touchPoint = gesture.location(in: self)
        let halfWidth = self.frame.size.width / 2
        if halfWidth > touchPoint.x {
            print("Left")
            delegate?.vgPlayerView(didDoubleTap: .left)
            
            UIView.animate(withDuration: 0.4) {
                self.reverseControlButton.alpha = 1.0
            } completion: { _ in
                UIView.animate(withDuration: 0.4) {
                    self.reverseControlButton.alpha = 0.0
                }
            }
            
            if let duration = vgPlayer?.totalDuration {
                totalDuration = duration
                current = Double(vgPlayer?.currentDuration ?? 0) - 10
                if current == (vgPlayer?.totalDuration ?? 0) {
                    current = vgPlayer?.totalDuration ?? 0
                }
            }
        } else {
            print("Right")
            delegate?.vgPlayerView(didDoubleTap: .right)
            
            UIView.animate(withDuration: 0.4) {
                self.forwardControlButton.alpha = 1.0
            } completion: { _ in
                UIView.animate(withDuration: 0.4) {
                    self.forwardControlButton.alpha = 0.0
                }
            }
        ...
        vgPlayer?.seekTime(current, completion: { [weak self] (finished) in
            guard let strongSelf = self else { return }
            if finished {
                strongSelf.isTimeSliding = false
                strongSelf.setupTimer()
        ...
        timeLabel.text = "\(formatSecondsToString(current) + " / " +  (formatSecondsToString(totalDuration)))"
...
extension VGPlayerView {
    ...
    internal func configurationTopView() {
        ...
        let closeImage = VGPlayerUtils.imageResource("back_n")
        closeButton.setImage(VGPlayerUtils.imageSize(image: closeImage!, scaledToSize: CGSize(width: 25, height: 20)), for: .normal)
    ...
    internal func configurationMiddleControlView() {
        addSubview(playControlButton)
        let playImage = VGPlayerUtils.imageResource("Icon - play-circle")
        let pauseImage = VGPlayerUtils.imageResource("Icon - pause")
        playControlButton.setImage(VGPlayerUtils.imageSize(image: playImage!, scaledToSize: CGSize(width: 25, height: 25)), for: .normal)
        playControlButton.setImage(VGPlayerUtils.imageSize(image: pauseImage!, scaledToSize: CGSize(width: 25, height: 25)), for: .selected)
        playControlButton.addTarget(self, action: #selector(onPlayerButton(_:)), for: .touchUpInside)
        
        
        addSubview(forwardControlButton)
        let forwardImage = VGPlayerUtils.imageResource("forward")
        forwardControlButton.isUserInteractionEnabled = false
        forwardControlButton.contentHorizontalAlignment = .fill
        forwardControlButton.contentVerticalAlignment = .fill
        forwardControlButton.setImage(forwardImage, for: .normal)
        
        addSubview(reverseControlButton)
        let reverseImage = VGPlayerUtils.imageResource("reverse")
        reverseControlButton.isUserInteractionEnabled = false
        reverseControlButton.contentHorizontalAlignment = .fill
        reverseControlButton.contentVerticalAlignment = .fill
        reverseControlButton.setImage(reverseImage, for: .normal)
    }
    ...
    internal func configurationBottomView() {
        ...
        timeSlider.minimumTrackTintColor = UIColor(named: "Color2")!
        timeSlider.thumbTintColor = UIColor(named: "Color2")!
        ...
        let enlargeImage = VGPlayerUtils.imageResource("maximize")
        let narrowImage = VGPlayerUtils.imageResource("minimize")
        ...
        fullscreenButton.tintColor = UIColor(named: "Color2")!
    ...
    internal func setupViewAutoLayout() {
        ...
        playButtion.snp.makeConstraints { [weak self] (make) in
            guard let strongSelf = self else { return }
            make.left.equalTo(strongSelf.bottomView).offset(0)
        ...
        playControlButton.snp.makeConstraints { [weak self] make in
            guard let strongSelf = self else { return }
            make.center.equalTo(strongSelf)
        ...
        forwardControlButton.translatesAutoresizingMaskIntoConstraints = false
        reverseControlButton.translatesAutoresizingMaskIntoConstraints = false
        
        NSLayoutConstraint.activate([
            self.forwardControlButton.leadingAnchor.constraint(equalTo: playControlButton.trailingAnchor, constant: 75),
            self.forwardControlButton.centerYAnchor.constraint(equalTo: playControlButton.centerYAnchor, constant: 0),
            self.forwardControlButton.widthAnchor.constraint(equalToConstant: 26),
            self.forwardControlButton.heightAnchor.constraint(equalToConstant: 26),
            
            self.reverseControlButton.trailingAnchor.constraint(equalTo: playControlButton.leadingAnchor, constant: -75),
            self.reverseControlButton.centerYAnchor.constraint(equalTo: playControlButton.centerYAnchor, constant: 0),
            self.reverseControlButton.widthAnchor.constraint(equalToConstant: 26),
            self.reverseControlButton.heightAnchor.constraint(equalToConstant: 26),
        ])
    }
}
